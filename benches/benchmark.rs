...
fn bench_names(c: &mut criterion::Criterion) {
    c.bench_function("names", |b| b.iter(names));
    c.bench_function("unicode", |b| b.iter(unicode));
}

criterion::criterion_group!(benches, bench_names);
